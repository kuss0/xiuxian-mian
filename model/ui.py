...
from .features.guanxing import get_guanxing_status_text, get_guanxing_summary_text
...
def get_ui_snapshot(session_token=None):
    ...
    return {
        ...
        "global_enabled": get_global_enabled(),
        "guanxing_enabled": get_guanxing_enabled(),
        "guanxing_summary": get_guanxing_summary_text(),
        "guanxing_detail": get_guanxing_status_text(),
...
def render_ui_page(message="", selected_send_as_id=None, session_token=None):
    ...
    return (
        ...
        "      <label class='field-label'><span>观星</span><label class='toggle-field'><input type='checkbox' name='guanxing_enabled' /><span>开启后，全局监听群内星盘显化广播，并按 3 小时时段收口</span></label></label>"
        ...
        "function renderSummary(identity){const panel=document.getElementById('summary-panel');const gxEnabled=!!(appState.snapshot&&appState.snapshot.guanxing_enabled);const gxSwitchClass=gxEnabled?'on':'off';const gxSwitchText=gxEnabled?'已开启':'已关闭';const gxNextEnabled=gxEnabled?0:1;const gxSummary=escapeHtml(appState.snapshot&&appState.snapshot.guanxing_summary||'');const gxDetail=escapeHtml(appState.snapshot&&appState.snapshot.guanxing_detail||'暂无详情');if(!identity){panel.innerHTML=`<section class=\"card\"><div class=\"summary-head\"><h2>全局观星</h2><div class=\"summary-head-actions\"><button type=\"button\" class=\"switch switch-${gxSwitchClass}\" data-toggle-module=\"1\" data-module=\"观星\" data-enabled=\"${gxNextEnabled}\"><span class=\"switch-track\"><span class=\"switch-thumb\"></span></span><span class=\"switch-text\">${gxSwitchText}</span></button></div></div><div class=\"meta\">当前没有可展示的角色ID。</div><pre>${gxDetail}</pre></section>`;return;}const sortedModules=(identity.modules||[]).slice().sort((a,b)=>Number(Boolean(b.enabled))-Number(Boolean(a.enabled)));const moduleSummary=sortedModules.map(module=>{const effectiveMark=module.effective_enabled?'🟢':(module.enabled?'🟡':'🔴');const suffix=module.enabled&&(!module.effective_enabled)?'（暂停中）':'';return `<span class=\\\"summary-chip\\\">${effectiveMark} ${escapeHtml(module.name)}${escapeHtml(suffix)}</span>`;}).join('');const refreshDisabled=identity.sect_refresh_pending?' disabled':'';const refreshText=identity.sect_refresh_pending?'更新中':'更新信息';const identityNextEnabled=identity.identity_enabled?0:1;const identitySwitchClass=identity.identity_enabled?'on':'off';const identitySwitchText=identity.identity_enabled?'运行中':'已暂停';const xwCur=identity.xiuwei_current||0;const xwMax=identity.xiuwei_max||0;const xwPct=xwMax>0?Math.min(100,Math.round(xwCur/xwMax*10000)/100):0;const xwBarHtml=xwMax>0?`<div class=\\\"xiuwei-bar-wrap\\\"><div class=\\\"xiuwei-bar-label\\\">修为</div><div class=\\\"xiuwei-bar-track\\\"><div class=\\\"xiuwei-bar-fill\\\" style=\\\"width:${xwPct}%\\\"></div><span class=\\\"xiuwei-bar-text\\\">${xwPct}%（${xwCur.toLocaleString()} / ${xwMax.toLocaleString()}）</span></div></div>`:'';panel.innerHTML=`<section class=\"card\"><div class=\"summary-head\"><h2>全局观星</h2><div class=\"summary-head-actions\"><button type=\"button\" class=\"switch switch-${gxSwitchClass}\" data-toggle-module=\"1\" data-module=\"观星\" data-enabled=\"${gxNextEnabled}\"><span class=\"switch-track\"><span class=\"switch-thumb\"></span></span><span class=\"switch-text\">${gxSwitchText}</span></button></div></div><div class=\"meta\">当前结果：${gxSummary}</div><pre>${gxDetail}</pre></section><section class=\"card\"><div class=\"summary-head\"><h2>${escapeHtml(identity.display_name)}</h2><div class=\"summary-head-actions\"><button type=\"button\" class=\"switch switch-${identitySwitchClass}\" data-toggle-identity=\"${identity.send_as_id}\" data-enabled=\"${identityNextEnabled}\"><span class=\"switch-track\"><span class=\"switch-thumb\"></span></span><span class=\"switch-text\">${identitySwitchText}</span></button><span class=\"form-label\" style=\"margin:0;\">信息更新时间：${escapeHtml(identity.sect_updated_at||'未设置')}</span><button type=\"button\" class=\"btn btn-secondary\" data-refresh-identity=\"${identity.send_as_id}\"${refreshDisabled}>${refreshText}</button><button type=\"button\" class=\"btn btn-secondary\" data-delete-identity=\"${identity.send_as_id}\">删除身份</button></div></div><div class=\"meta\">主账号：${escapeHtml(appState.snapshot.account_user_id||'未获取')} ｜ username: @${escapeHtml(identity.username||'未设置')} ｜ 角色名: ${escapeHtml(identity.label||'未设置')} ｜ 道号: ${escapeHtml(identity.daohao||'未获取')} ｜ 角色ID: ${escapeHtml(identity.send_as_id)} ｜ 境界: ${escapeHtml(identity.realm||'未获取')} ｜ ${escapeHtml(getSectStatusText(identity))}</div>${xwBarHtml}<div class=\"summary-strip\">${moduleSummary}</div></section>`;}"
...
async def ui_set_basic_config(game_group_id, game_bot_ids, game_topic_id, auto_delete_sent_messages, guanxing_enabled=False, actor_id=None):
    ...
    guanxing_switch_enabled = bool(guanxing_enabled)
    ...
    set_guanxing_enabled(guanxing_switch_enabled)
    save_state()
    actor_suffix = f"｜操作者：{actor_id}" if actor_id is not None else ""
    ...
    display_guanxing = "开启" if guanxing_switch_enabled else "关闭"
    await send_audit_log(
        f"🧩 已更新基础配置：群={group_id}｜bot={display_bots}｜话题={display_topic}｜自动删消息={display_auto_delete}｜观星={display_guanxing}{actor_suffix}",
        scope="global",
        limit=320,
    )
    return True, f"已更新基础配置：群聊 {group_id} ｜ bot {display_bots} ｜ 话题 {display_topic} ｜ 自动删消息 {display_auto_delete} ｜ 观星 {display_guanxing}"
        ...
        if path == "/api/login/exchange":
            ...
            if path == "/favicon.png":
                ...
                if session is None:
                    ...
                    ok, message = await ui_set_basic_config(payload.get("game_group_id"), payload.get("game_bot_ids"), payload.get("game_topic_id"), payload.get("auto_delete_sent_messages"), payload.get("guanxing_enabled"), actor_id=(session or {}).get("sender_id"))
