...
def render_ui_page(message="", selected_send_as_id=None, session_token=None):
    ...
    return (
        ...
        "async function syncStargazerSlots(){try{const data=await postJson('/api/stargazer-sync',{send_as_id:appState.selectedId});updateFlash(data.message||'已发送观星监控台同步指令',false);applySnapshot(data.snapshot||appState.snapshot,{keepFlash:true});}catch(error){updateFlash((error&&error.message)||'同步星盘失败',true);renderAll();}}"
