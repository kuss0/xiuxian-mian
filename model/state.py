...
IDENTITY_RUNTIME_COLUMNS = [
    ...
    "nanlong_reply_to_msg_id", "nanlong_reply_chat_id", "nanlong_prompt_at", "nanlong_last_prompt_key", "nanlong_reply_due_at", "nanlong_last_msg_id", "nanlong_last_chat_id", "nanlong_last_sent_at", "nanlong_retry_count", "nanlong_last_command", "nanlong_protect_phase", "nanlong_place_msg_id", "nanlong_recall_msg_id", "nanlong_last_error",
    ...
    "explore_rift_reply_to_msg_id", "explore_rift_reply_due_at", "explore_rift_pending_result_msg_id", "explore_rift_last_msg_id", "explore_rift_last_result", "explore_rift_last_error", "explore_rift_last_result_key", "explore_rift_result_evidence", "explore_rift_manual_required", "explore_rift_tianxing_prepare_retry_at",
    "explore_rift_nascent_escape_weak_until", "explore_rift_rebirth_required", "explore_rift_rebirth_operation", "explore_rift_rebirth_phase", "explore_rift_rebirth_due_at", "explore_rift_rebirth_request_msg_id", "explore_rift_rebirth_options_msg_id", "explore_rift_rebirth_select_msg_id", "explore_rift_rebirth_options_text", "explore_rift_rebirth_selected_index", "explore_rift_rebirth_last_result", "explore_rift_rebirth_last_error", "explore_rift_rebirth_choice_mode", "explore_rift_rebirth_preferred_root_type", "explore_rift_rebirth_preferred_attrs", "explore_rift_rebirth_blind_index", "explore_rift_fatal_msg_id", "explore_rift_fatal_confirm_due_at",
    ...
    "duel_target", "duel_total_count", "duel_completed_count", "duel_reserve_xiuwei", "duel_window_start_minute", "duel_window_end_minute", "duel_daily_limit_day", "duel_daily_limited_targets", "duel_log_reconcile_day", "duel_log_reconcile_at", "duel_observed_completed_count", "duel_observed_baseline_count", "duel_observed_manual_count", "duel_observed_mind_remaining", "duel_observed_last_command_msg_id", "duel_reply_to_msg_id", "duel_reply_due_at", "duel_open_msg_id", "duel_magic_due_at", "duel_magic_sent_at", "duel_started_at", "duel_phaseful_retry_count", "duel_last_msg_id", "duel_last_result", "duel_last_error", "duel_unequip_prepared",
    ...
    "second_soul_phase", "second_soul_commands", "second_soul_choice_strategy", "second_soul_heart_demon_msg_id", "second_soul_heart_demon_chat_id", "second_soul_heart_demon_account_id", "second_soul_heart_demon_choice_msg_id", "second_soul_heart_demon_notified", "second_soul_status_msg_id", "second_soul_train_msg_id",
...
REPLICA_KIND_CONFIG_DEFAULTS = {
    kind: copy.deepcopy(REPLICA_KIND_CONFIG_TEMPLATE)
    for kind in REPLICA_KIND_KEYS
}
...
IDENTITY_STATE_TEMPLATE = {
    ...
    "small_world_high_stock_silence_enabled": False,
    ...
    "concubine_last_panel_chat_id": 0,
    "concubine_name": "",
    "concubine_kind": "",
    "concubine_location": "",
    ...
    "nanlong_reply_chat_id": 0,
    "nanlong_prompt_at": 0,
    "nanlong_last_prompt_key": "",
    "nanlong_reply_due_at": 0,
    "nanlong_last_msg_id": 0,
    "nanlong_last_chat_id": 0,
    "nanlong_last_sent_at": 0,
    ...
    "duel_daily_limit_day": "",
    "duel_daily_limited_targets": [],
    "duel_log_reconcile_day": "",
    "duel_log_reconcile_at": 0,
    "duel_observed_completed_count": 0,
    "duel_observed_baseline_count": 0,
    "duel_observed_manual_count": 0,
    "duel_observed_mind_remaining": -1,
    "duel_observed_last_command_msg_id": 0,
    ...
    "fishing_daily_catch_summary_json": "",
    "fishing_daily_summary_day": "",
    ...
    "second_soul_phase": "idle",  # idle|status_pending|ready_to_train|train_pending|cultivating|heart_demon_pending|injured|not_unlocked|purge_ready|purge_pending|purge_status_pending
    "second_soul_commands": {},
    "second_soul_choice_strategy": "stable",
    "next_second_soul_time": 0,
    "second_soul_heart_demon_msg_id": 0,
    "second_soul_heart_demon_chat_id": 0,
    "second_soul_heart_demon_account_id": 0,
    "second_soul_heart_demon_choice_msg_id": 0,
...
GLOBAL_STATE_DEFAULTS = {
    ...
    "replica_kind_configs": copy.deepcopy(REPLICA_KIND_CONFIG_DEFAULTS),
    "replica_run_state": {},
...
def get_miniapp_auto_config():
    records = _meta_state.get("miniapp_auto_config") or {}
    return records if isinstance(records, dict) else {}
...
def set_miniapp_auto_config(records):
    _meta_state["miniapp_auto_config"] = records if isinstance(records, dict) else {}
    return get_miniapp_auto_config()
...
def _normalize_replica_kind_config(config):
    config = config if isinstance(config, dict) else {}
    return {
        "enabled": _coerce_meta_bool(config.get("enabled"), True),
        "participant_identity_ids": _normalize_replica_participant_identity_ids(config.get("participant_identity_ids") or []),
        "dispatch_participant_identity_ids": _normalize_replica_participant_identity_ids(config.get("dispatch_participant_identity_ids") or []),
    }


def _normalize_replica_kind_configs(configs):
    configs = configs if isinstance(configs, dict) else {}
    normalized = {}
    for kind in REPLICA_KIND_KEYS:
        normalized[kind] = _normalize_replica_kind_config(configs.get(kind) or {})
    return normalized


def get_replica_kind_configs():
    normalized = _normalize_replica_kind_configs(_meta_state.get("replica_kind_configs") or {})
    _meta_state["replica_kind_configs"] = normalized
    return copy.deepcopy(normalized)


def set_replica_kind_configs(configs):
    _meta_state["replica_kind_configs"] = _normalize_replica_kind_configs(configs)
    return get_replica_kind_configs()


def get_replica_kind_config(replica_kind):
    replica_kind = str(replica_kind or "").strip()
    return copy.deepcopy(get_replica_kind_configs().get(replica_kind) or _normalize_replica_kind_config({}))
...
def _normalize_game_group_route_config(value):
    raw = dict(value or {}) if isinstance(value, dict) else {}
    primary = int(raw.get("primary_group_id") or get_game_group_id() or 0)
    backups = []
    for item in raw.get("backup_group_ids") or ():
        try:
            group_id = int(item)
        except (TypeError, ValueError, OverflowError):
            continue
        if group_id and group_id != primary and group_id not in backups:
            backups.append(group_id)
    raw_topics = raw.get("topic_id_by_group") or {}
    topics = {}
    if isinstance(raw_topics, dict):
        for raw_group_id, raw_topic_id in raw_topics.items():
            try:
                group_id = int(raw_group_id)
                topic_id = max(0, int(raw_topic_id or 0))
            except (TypeError, ValueError, OverflowError):
                continue
            if group_id:
...
def get_game_group_route_config():
...
def set_game_group_route_config(config):
    normalized = _normalize_game_group_route_config(config)
    _meta_state["game_group_route_config"] = normalized
    if normalized.get("primary_group_id"):
        _meta_state["game_group_id"] = int(normalized["primary_group_id"])
    return get_game_group_route_config()
...
def is_game_group_id(group_id):
    try:
        return int(group_id or 0) in set(get_game_group_ids())
    except (TypeError, ValueError, OverflowError):
        return False


def get_game_group_topic_id(group_id, default=None):
    try:
        group_id = int(group_id or 0)
    except (TypeError, ValueError, OverflowError):
        return int(get_game_topic_id() if default is None else default or 0)
    config = get_game_group_route_config()
    topics = config.get("topic_id_by_group") or {}
    if str(group_id) in topics:
        return int(topics[str(group_id)] or 0)
    return int(get_game_topic_id() if default is None else default or 0)
...
def set_game_group_id(group_id):
    ...
    config = _normalize_game_group_route_config(_meta_state.get("game_group_route_config") or {})
    config["primary_group_id"] = int(group_id or 0)
    if int(group_id or 0):
        config.setdefault("topic_id_by_group", {})[str(int(group_id))] = get_game_topic_id()
    _meta_state["game_group_route_config"] = config
    _meta_state["replica_dispatch_group_ids"] = _normalize_replica_dispatch_group_ids(
        _meta_state.get("replica_dispatch_group_ids") or []
    )
...
def set_game_topic_id(topic_id):
    ...
    primary_group_id = get_game_group_id()
    if primary_group_id:
        config = _normalize_game_group_route_config(_meta_state.get("game_group_route_config") or {})
        config.setdefault("topic_id_by_group", {})[str(primary_group_id)] = get_game_topic_id()
        _meta_state["game_group_route_config"] = config
    return get_game_topic_id()


def _normalize_account_group_memberships(records):
    normalized = {}
    if not isinstance(records, dict):
        return normalized
    for raw_key, raw_record in records.items():
        ...
        try:
            account_id = int(raw_record.get("account_id") or key_parts[0] or 0)
            game_group_id = int(raw_record.get("game_group_id") or (key_parts[1] if len(key_parts) > 1 else 0) or 0)
        except (TypeError, ValueError, OverflowError):
            continue
        if account_id <= 0 or game_group_id == 0:
            continue
        status = str(raw_record.get("status") or "unknown").strip().lower()
        probe_status = str(raw_record.get("probe_status") or status).strip().lower()
        allowed = {"member", "not_member", "not_applicable", "unknown"}
        if status not in allowed:
            status = "unknown"
        if probe_status not in allowed:
        ...
        for raw_identity_id in raw_record.get("identity_ids") or ():
            ...
            if identity_id > 0:
                identity_ids.append(identity_id)
        normalized[f"{account_id}:{game_group_id}"] = {
            "account_id": account_id,
            "game_group_id": game_group_id,
            "identity_ids": sorted(set(identity_ids)),
            "status": status,
...
def get_account_group_memberships():
    records = _normalize_account_group_memberships(_meta_state.get("account_group_memberships") or {})
    _meta_state["account_group_memberships"] = records
    return copy.deepcopy(records)


def set_account_group_memberships(records):
    _meta_state["account_group_memberships"] = _normalize_account_group_memberships(records)
    return get_account_group_memberships()


def get_account_group_membership(account_id, game_group_id):
    try:
        key = f"{int(account_id or 0)}:{int(game_group_id or 0)}"
    except (TypeError, ValueError, OverflowError):
        return {}
    return dict(get_account_group_memberships().get(key) or {})


def set_account_group_membership(account_id, game_group_id, record):
    try:
        account_id = int(account_id or 0)
        game_group_id = int(game_group_id or 0)
    except (TypeError, ValueError, OverflowError):
        return {}
    records = get_account_group_memberships()
    key = f"{account_id}:{game_group_id}"
    if account_id <= 0 or game_group_id == 0 or not isinstance(record, dict):
        records.pop(key, None)
    else:
        records[key] = {**record, "account_id": account_id, "game_group_id": game_group_id}
    set_account_group_memberships(records)
    return get_account_group_membership(account_id, game_group_id)


def get_world_boss_rotation_state():
    records = _meta_state.get("world_boss_rotation_state") or {}
    return copy.deepcopy(records) if isinstance(records, dict) else {}


def set_world_boss_rotation_state(records):
    _meta_state["world_boss_rotation_state"] = records if isinstance(records, dict) else {}
    return get_world_boss_rotation_state()
...
def get_global_pause_source():
    return str(_meta_state.get("global_pause_source") or "")
