...
class CheckinNoSectTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_checkin_runtime_block_is_not_reported_as_send_failure(self):
        ...
        now = datetime(2026, 6, 20, 2, 30, tzinfo=timezone.utc).timestamp()
        state_module.ensure_identity_registered(send_as_id)
        state_module.update_send_as_profile(send_as_id, username="checkinblocked", sect_name="星宫")
        ...
        with state_module.use_identity(send_as_id) as identity_state:
            ...
            with (
                ...
                patch.object(checkin, "send_game_command", new=AsyncMock(return_value=None)),
                patch.object(checkin, "classify_game_send_block", return_value={
                    "status": "unsent",
                    "code": "global_recovery_cooldown",
                }),
                patch.object(checkin, "save_state"),
                patch.object(checkin, "console_log") as console_mock,
                patch.object(checkin, "send_audit_log", new=AsyncMock()) as audit_mock,
            ):
                await checkin.run_checkin_scheduler(now)

            self.assertEqual(now + 1 + checkin.RETRY_MAX_SEC, identity_state["next_checkin_time"])
    ...
    async def test_sect_teach_runtime_block_is_not_reported_as_send_failure(self):
        ...
        now = datetime(2026, 6, 20, 2, 30, tzinfo=timezone.utc).timestamp()
        state_module.ensure_identity_registered(send_as_id)
        state_module.update_send_as_profile(send_as_id, username="teachblocked", sect_name="星宫")
        ...
        with state_module.use_identity(send_as_id) as identity_state:
            ...
            self.assertEqual(now + 1 + checkin.RETRY_MAX_SEC, identity_state["next_sect_teach_time"])
            self.assertEqual(8808, identity_state["sect_teach_reply_to_msg_id"])
            self.assertIn("传功未发送", str(console_mock.call_args.args[0]))
            audit_mock.assert_not_awaited()
