...
class StartupRecoveryGuardTests(unittest.TestCase):
    ...
    def test_initialize_explore_rift_preserves_inflight_recovery(self):
        now = 1_700_000_000.0
        send_as_id = self._prepare_identity()
        with state_module.use_identity(send_as_id):
            self._disable_modules()
        ...
        with state_module.use_identity(send_as_id):
            self.assertEqual(10425942, state_module.state["explore_rift_reply_to_msg_id"])
            self.assertEqual(now - 1, state_module.state["explore_rift_reply_due_at"])
            self.assertEqual(10425944, state_module.state["explore_rift_pending_result_msg_id"])
            self.assertEqual(0, state_module.state["next_explore_rift_time"])
            self.assertEqual("保留上一条异常", state_module.state["explore_rift_last_error"])

    def test_initialize_explore_rift_without_pending_schedules_initial_check(self):
        now = 1_700_000_000.0
        send_as_id = self._prepare_identity()
        with state_module.use_identity(send_as_id):
            self._disable_modules()
        ...
        with state_module.use_identity(send_as_id):
            ...
            self.assertEqual(now + explore_rift.EXPLORE_RIFT_RECOVERY_MIN_SEC, state_module.state["next_explore_rift_time"])
    ...
    def test_archived_tree_recovery_clears_normal_due_timer(self):
        now = 1_700_000_000.0
        send_as_id = self._prepare_identity()
        with state_module.use_identity(send_as_id):
            self._disable_modules()
        ...
        with state_module.use_identity(send_as_id):
            ...
            self.assertEqual(0, state_module.state["next_irr_time"])
            self.assertEqual(0, state_module.state["last_tree_status_sent_at"])
    ...
    def test_tree_recovery_releases_stale_harvested_maturing_state(self):
        ...
        with state_module.use_identity(send_as_id):
            ...
            self.assertEqual(0, state_module.state["tree_last_harvest_result_msg_id"])
            self.assertEqual(0, state_module.state["tree_last_harvest_reply_to_msg_id"])
            self.assertEqual(0, state_module.state["next_irr_time"])

    def test_archived_tree_recovery_clears_invasion_status_probe(self):
        now = 1_700_000_000.0
        send_as_id = self._prepare_identity()
        with state_module.use_identity(send_as_id):
            self._disable_modules()
        ...
        with state_module.use_identity(send_as_id):
            self.assertFalse(state_module.state["tree_enabled"])
            self.assertFalse(state_module.state["is_invading"])
            self.assertEqual(0, state_module.state["next_irr_time"])
    ...
    def test_deep_retreat_summary_due_with_command_history_recovery_wakes_immediately(self):
        now = 1_700_000_000.0
        send_as_id = self._prepare_identity()
        with state_module.use_identity(send_as_id):
            self._disable_modules()
            state_module.state["deep_retreat_enabled"] = True
            state_module.state["deep_retreat_phase"] = "summary_due"
            state_module.state["deep_retreat_summary_sent_at"] = now - 600
            state_module.state["last_deep_retreat_command_time"] = now - 3600
            state_module.state["last_deep_retreat_summary_msg_id"] = 0
            state_module.state["next_deep_retreat_time"] = now + 120
    ...
    def test_phaseful_summary_due_startup_spread_stays_immediate(self):
        now = 1_700_000_000.0
        send_as_id = self._prepare_identity()
        with state_module.use_identity(send_as_id):
            self._disable_modules()
        ...
        with state_module.use_identity(send_as_id):
            self.assertEqual("summary_due", state_module.state["deep_retreat_phase"])
            self.assertEqual(now + 1, state_module.state["next_deep_retreat_time"])
        self.assertFalse(uniform_mock.called)
