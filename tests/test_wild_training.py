...
class WildTrainingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_timeout_enters_unknown_window_without_fast_retry(self):
        ...
        with state_module.use_identity(send_as_id), \
             ...
             patch.object(wild_training, "classify_game_send_block", return_value={"status": "unknown", "code": "send_timeout"}), \
    ...
    async def test_duplicate_final_edit_is_idempotent(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_010.0
        text = (
            "【野外历练 · 灵机暗藏】\n"
    ...
    async def test_completed_anchor_blocks_stale_due_after_tianxing_status_overwrites_result(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_100.0
        result_at = now - wild_training.WILD_TRAINING_CYCLE_MIN_SEC + 150
        with state_module.use_identity(send_as_id) as identity_state:
            identity_state["wild_training_reply_to_msg_id"] = 0
            identity_state["wild_training_reply_due_at"] = 0
            identity_state["wild_training_retry_count"] = 0
            identity_state["wild_training_last_msg_id"] = 201
            identity_state["wild_training_last_result"] = "天星时间线：sent_waiting_ack"
            identity_state["wild_training_last_result_at"] = 0
            identity_state["wild_training_last_completed_at"] = result_at
            identity_state["next_wild_training_time"] = now - 1
    ...
    async def test_cleared_missing_edit_state_recovers_late_result_from_message_log(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_760.0
        result_text = (
            "【野外历练 · 妖兽遭遇】\n"
        ...
        with tempfile.TemporaryDirectory() as log_dir:
            ...
            with state_module.use_identity(send_as_id) as identity_state:
                ...
                identity_state["wild_training_retry_count"] = 0
                identity_state["wild_training_last_msg_id"] = 201
                identity_state["wild_training_last_result"] = "结果编辑未留存，已按正常周期恢复，原消息ID=201"
                identity_state["wild_training_last_result_at"] = now - 60
                identity_state["wild_training_last_completed_at"] = now - 60
                identity_state["wild_training_last_error"] = ""

            with state_module.use_identity(send_as_id), \
                 patch.object(wild_training, "MESSAGES_DIR", log_dir), \
                 patch.object(wild_training, "save_state"), \
                 patch.object(wild_training.random, "uniform", return_value=wild_training.WILD_TRAINING_CYCLE_MIN_SEC), \
                 patch.object(wild_training, "send_audit_log", new=AsyncMock()) as audit_mock:
                await wild_training.run_wild_training_scheduler(now)
        ...
        self.assertEqual(0, state_module.state["wild_training_reply_to_msg_id"])
        self.assertEqual(201, state_module.state["wild_training_last_msg_id"])
        self.assertEqual("修为+12000 ｜ 奖励:三级妖丹x1", state_module.state["wild_training_last_result"])
        self.assertEqual(now - 5, state_module.state["wild_training_last_completed_at"])
        self.assertEqual(now - 5 + wild_training.WILD_TRAINING_CYCLE_MIN_SEC, state_module.state["next_wild_training_time"])
    ...
    async def test_tianxing_started_timeout_invalidates_uncertain_explore_release(self):
        ...
        self.assertIn("天星探索结算未确认", args[0])
        self.assertIn("本轮不计收益", args[0])
    ...
    async def test_scheduler_send_queue_timeout_defers_without_reply_retry(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        ...
        with state_module.use_identity(send_as_id), \
             patch.object(wild_training.random, "uniform", return_value=wild_training.WILD_TRAINING_RETRY_MIN_SEC), \
             patch.object(wild_training, "send_game_command", new=AsyncMock(return_value=None)) as send_mock, \
             patch.object(wild_training, "classify_game_send_block", return_value={"status": "unsent", "code": "send_queue_timeout"}), \
             patch.object(wild_training, "send_audit_log", new=AsyncMock()) as audit_mock, \
             patch.object(wild_training, "save_state"):
            await wild_training.run_wild_training_scheduler(now)

        send_mock.assert_awaited_once()
        self.assertEqual(wild_training.WILD_TRAINING_SEND_TIMEOUT_SEC, send_mock.await_args.kwargs["queue_timeout"])
        self.assertEqual(0, state_module.state["wild_training_reply_to_msg_id"])
        self.assertEqual(0, state_module.state["wild_training_reply_due_at"])
        self.assertEqual(0, state_module.state["wild_training_retry_count"])
        self.assertEqual(now + wild_training.WILD_TRAINING_RETRY_MIN_SEC, state_module.state["next_wild_training_time"])
    ...
    async def test_scheduler_action_guard_short_window_defers_without_consuming_retry(self):
        ...
        with state_module.use_identity(send_as_id), \
             patch.object(wild_training, "send_game_command", new=AsyncMock(return_value=None)) as send_mock, \
             patch.object(wild_training, "classify_game_send_block", return_value={"status": "unsent", "code": "action_guard", "reason": "野外历练 同命令短窗保护，剩余约 26s"}), \
    ...
    async def test_scheduler_ignores_deep_retreat_summary_window(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        sent = SimpleNamespace(id=12344, sent_at=now)
        with state_module.use_identity(send_as_id) as identity_state:
            identity_state["wild_training_reply_to_msg_id"] = 0
            identity_state["wild_training_reply_due_at"] = 0
            identity_state["wild_training_retry_count"] = 0
            ...
            identity_state["next_deep_retreat_time"] = now + 30
        ...
        self.assertEqual(sent.id, state_module.state["wild_training_reply_to_msg_id"])
        self.assertEqual("已发送：谨慎", state_module.state["wild_training_last_result"])

    async def test_scheduler_ignores_deep_retreat_window_for_tianxing_preflight(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        ...
        with state_module.use_identity(send_as_id) as identity_state:
            ...
            identity_state["next_deep_retreat_time"] = now + 180

        with state_module.use_identity(send_as_id), \
             patch.object(wild_training, "send_game_command", new=AsyncMock(return_value=sent)) as send_mock, \
             patch.object(wild_training, "_prepare_wild_training_tianxing_route", new=AsyncMock(return_value=True)), \
             patch.object(wild_training, "save_state"):
            await wild_training.run_wild_training_scheduler(now)
    ...
    async def test_scheduler_ignores_deep_retreat_launching_phase(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        ...
        with state_module.use_identity(send_as_id) as identity_state:
            ...
            identity_state["next_wild_training_time"] = now - 1
            identity_state["deep_retreat_enabled"] = True
            identity_state["deep_retreat_phase"] = "launching"
    ...
    async def test_scheduler_repairs_persisted_legacy_deep_retreat_deferral(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        with state_module.use_identity(send_as_id) as identity_state:
            identity_state["wild_training_reply_to_msg_id"] = 0
        ...
        self.assertEqual("已发送：谨慎", state_module.state["wild_training_last_result"])
        self.assertEqual("", state_module.state["wild_training_last_error"])
        self.assertTrue(any("解除深度闭关" in str(call) for call in console_mock.call_args_list))
    ...
    async def test_scheduler_falls_back_to_cautious_when_explore_prediction_has_no_change_fate_to_apply(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        with state_module.use_identity(send_as_id) as identity_state:
            state_module.update_send_as_profile(send_as_id, sect_name="天星宗")
    ...
    async def test_scheduler_releases_overdue_wild_training_as_cautious_after_tianxing_ack_timeout(self):
        ...
        sent_msg = SimpleNamespace(id=204, sent_at=now)

        with state_module.use_identity(send_as_id), \
             patch.object(wild_training, "run_tianxing_timeline_scheduler", new=AsyncMock(return_value={"phase": "ack_timeout", "changed": True})) as timeline_mock, \
             patch.object(wild_training, "send_game_command", new=AsyncMock(return_value=sent_msg)) as send_mock, \
             patch.object(wild_training, "console_log"), \
             patch.object(wild_training, "save_state"):
            await wild_training.run_wild_training_scheduler(now)
    ...
    async def test_scheduler_releases_overdue_wild_training_as_cautious_after_tianxing_send_blocked(self):
        ...
        with state_module.use_identity(send_as_id) as identity_state:
            ...
            identity_state["tianxing_observation"] = {
                ...
                "prediction_consumed_at": now - 120,
                "current_prediction_set_at": now - 300,
                "tianji_value": 42,
            }
            identity_state["tianxing_auto_config"] = {
        ...
        sent_msg = SimpleNamespace(id=206, sent_at=now)

        with state_module.use_identity(send_as_id), \
             patch.object(wild_training, "run_tianxing_timeline_scheduler", new=AsyncMock(return_value={"phase": "send_blocked", "changed": True})) as timeline_mock, \
             patch.object(wild_training, "send_game_command", new=AsyncMock(return_value=sent_msg)) as send_mock, \
             patch.object(wild_training, "console_log"), \
             patch.object(wild_training, "save_state"):
            await wild_training.run_wild_training_scheduler(now)
    ...
    async def test_scheduler_releases_overdue_wild_training_as_deep_after_tianxing_ack_timeout_with_change_fate(self):
        ...
        sent_msg = SimpleNamespace(id=205, sent_at=now)
    ...
    async def test_scheduler_uses_cautious_with_prediction_but_without_tianxing_change_fate(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        with state_module.use_identity(send_as_id) as identity_state:
            state_module.update_send_as_profile(send_as_id, sect_name="天星宗")
    ...
    async def test_scheduler_does_not_repeat_craft_prediction_consume_for_overdue_wild_training(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        sent_msg = SimpleNamespace(id=301, sent_at=now)
        with state_module.use_identity(send_as_id) as identity_state:
            state_module.update_send_as_profile(send_as_id, sect_name="天星宗")
            identity_state["wild_training_reply_to_msg_id"] = 0
            identity_state["wild_training_reply_due_at"] = 0
            ...
            identity_state["tianxing_timeline_state"] = {
                "craft_farm": {
                    ...
                    "last_command": ".炼制 玄铁剑",
                    "last_result": "炼制结束 玄铁剑",
                    "last_error": "炼制结算未见推命命中/落空，等待查盘校准。",
                },
            }

        with state_module.use_identity(send_as_id), \
             patch.object(wild_training, "run_tianxing_consume_craft_prediction", new=AsyncMock()) as consume_mock, \
             patch.object(wild_training, "run_tianxing_timeline_scheduler", new=AsyncMock()) as timeline_mock, \
             patch.object(wild_training, "send_game_command", new=AsyncMock(return_value=sent_msg)) as send_mock, \
             patch.object(wild_training, "save_state"):
            await wild_training.run_wild_training_scheduler(now)
    ...
    async def test_unknown_send_recovers_direct_result_from_message_log(self):
        ...
        self.assertIn("🌌 天星探索结果｜野外历练", first_args[0])
        self.assertEqual("high", first_kwargs["priority"])
        self.assertIn("🏞️ 野外历练日志补偿", second_args[0])
        self.assertEqual(0, state_module.state["wild_training_reply_to_msg_id"])
        self.assertEqual(0, state_module.state["wild_training_reply_due_at"])
        self.assertIn("天机+1", state_module.state["wild_training_last_result"])
    ...
    async def test_unknown_send_recovers_channel_identity_command_as_pending(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        command_ts = now - 120
        entries = [
            {
                ...
                "sender_id": int(f"-100{send_as_id}"),
                "reply_to_msg_id": 7310786,
                "text": ".野外历练 谨慎",
            },
        ]
        ...
        audit_mock.assert_awaited_once()
        self.assertEqual(501, state_module.state["wild_training_reply_to_msg_id"])
        self.assertEqual(501, state_module.state["wild_training_last_msg_id"])
        self.assertEqual(command_ts + wild_training.WILD_TRAINING_REPLY_TIMEOUT_SEC, state_module.state["wild_training_reply_due_at"])
        self.assertEqual("已发送：谨慎", state_module.state["wild_training_last_result"])
        self.assertIn("回捞到命令", state_module.state["wild_training_last_error"])

    async def test_unknown_send_without_tianxing_short_retries_when_command_not_found(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with state_module.use_identity(send_as_id), \
                 patch.object(wild_training, "MESSAGES_DIR", tmpdir), \
                 patch.object(wild_training, "save_state"), \
                 patch.object(wild_training.random, "uniform", return_value=wild_training.WILD_TRAINING_SEND_UNKNOWN_RETRY_MIN_SEC), \
                 patch.object(wild_training, "send_audit_log", new=AsyncMock()) as audit_mock, \
                 patch.object(wild_training, "send_game_command", new=AsyncMock()) as send_mock:
                await wild_training.run_wild_training_scheduler(now)
                await wild_training.run_wild_training_scheduler(now + 60)
        ...
        self.assertEqual(0, state_module.state["wild_training_reply_due_at"])
        self.assertEqual(0, state_module.state["wild_training_retry_count"])
        self.assertEqual("未知发送未找回，短退避后重试", state_module.state["wild_training_last_result"])
        self.assertIn("普通野外短退避后重试", state_module.state["wild_training_last_error"])
        self.assertEqual(now + wild_training.WILD_TRAINING_SEND_UNKNOWN_RETRY_MIN_SEC, state_module.state["next_wild_training_time"])

    async def test_unknown_send_with_tianxing_calibrates_and_short_retries_when_command_not_found(self):
        send_as_id = self._prepare_identity()
        now = 1_700_000_700.0
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with state_module.use_identity(send_as_id), \
                 ...
                 patch.object(wild_training, "save_state"), \
                 patch.object(wild_training.random, "uniform", return_value=wild_training.WILD_TRAINING_SEND_UNKNOWN_RETRY_MIN_SEC), \
                 patch.object(wild_training, "_send_tianxing_panel_calibration", new=AsyncMock(return_value=True)) as panel_mock, \
                 patch.object(wild_training, "mark_tianxing_route_result_unknown") as mark_unknown_mock, \
        ...
        send_mock.assert_not_awaited()
        panel_mock.assert_awaited_once()
        mark_unknown_mock.assert_called_once()
        audit_mock.assert_awaited_once()
        self.assertEqual(0, state_module.state["wild_training_reply_to_msg_id"])
        self.assertEqual(0, state_module.state["wild_training_reply_due_at"])
        self.assertEqual(0, state_module.state["wild_training_retry_count"])
        self.assertEqual("未知发送未找回，短退避后重试", state_module.state["wild_training_last_result"])
        self.assertIn("已查天机盘校准后短退避重试", state_module.state["wild_training_last_error"])
        self.assertEqual(now + wild_training.WILD_TRAINING_SEND_UNKNOWN_RETRY_MIN_SEC, state_module.state["next_wild_training_time"])
