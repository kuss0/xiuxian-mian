...
def record_timeline_test_receipt(identity_id, chat_id=-100530002):
    ...
    account_id = state_module.get_identity_account(identity_id)
    assert account_id > 0 and step["send_msg_id"] > 0 and step["sent_at"] > 0
    step.update(
        send_account_id=account_id, send_chat_id=chat_id,
        send_started_at=step.get("send_started_at") or step["sent_at"],
    )
    tianxing._set_timeline_step(timeline, tianxing._timeline_active_index(timeline), step)
    state_module.state["tianxing_timeline_state"] = timeline
    return {
        "send_as_id": identity_id, "chat_id": chat_id,
        "root_msg_id": step["send_msg_id"], "msg_id": step["send_msg_id"] + 1,
    }
...
class TianxingParserTests(unittest.TestCase):
    ...
    def test_tianxing_retreat_success_parses_prediction_gain_and_cooldown(self):
        ...
        self.assertNotIn("current_prediction_until", parsed)
        self.assertGreater(parsed["normal_retreat_next_time"], now + 10 * 60)

    def test_craft_farm_parses_tianxing_hit_without_clearing_prediction(self):
        now = 1_780_000_000.0
        parsed = tianxing.parse_tianxing_text(
            "炼制结束！\n"
            "共开炉 1 次，成功 1 次。\n"
    ...
    def test_existing_prediction_and_change_cooldown_real_text_calibrate_state(self):
        ...
        self.assertEqual("", predict.get("last_error", ""))
        self.assertEqual("改命", change["action"])
    ...
    def test_normalize_clears_stale_cooldown_errors(self):
        ...
        changed = tianxing.normalize_tianxing_observation({
            "last_action": "改命",
            "last_result": "cooldown",
            "last_error": "改命尚未耗尽",
            "current_change": "探索",
...
class TianxingManualPlanTests(unittest.TestCase):
    ...
    def test_change_fate_success_confirms_step_without_consuming_prediction(self):
        now = 1_780_000_000.0
        text = (
            "你以 3 点天机值强行撬动命数，为 【探索】 预留了一次改命回天的余地。\n"
            "这道后手将在 24 小时 内生效一次。"
        )
        active_step = {
            "id": "change_fate:探索:test",
            "action": "change_fate",
            "arg": "探索",
            "route": "探索",
            "command": ".改命 探索",
            "status": "sent_waiting_ack",
            "send_msg_id": 11356833,
            "sent_at": now - 2,
            "ack_due_at": now + 88,
        }
        with state_module.use_identity(self.identity_id):
            state_module.set_identity_account(self.identity_id, 7532)
            state_module.state["tianxing_enabled"] = True
            state_module.state["tianxing_observation"] = {
                "last_observed_at": now - 60,
                "current_prediction": "探索",
            ...
            state_module.state["tianxing_timeline_state"] = {
                ...
                "steps": [
                    dict(active_step),
                    {"action": "release_downstream", "arg": "探索", "route": "探索", "status": "pending", "release_basis": "change_fate"},
                ],
                "released_routes": {},
            }

            self.assertTrue(tianxing.apply_tianxing_passive(
                text, now=now, family="tianxing_change_fate",
                reply_context=record_timeline_test_receipt(self.identity_id),
            ))
            observed = tianxing.normalize_tianxing_observation(state_module.state["tianxing_observation"])
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
        ...
        self.assertEqual("confirmed", timeline["steps"][0]["status"])
        self.assertEqual("state_confirmed", timeline["audit"][-1]["event"])

    def test_normalize_preserves_prediction_consumption_after_change_fate_reply(self):
        now = 1_780_000_000.0
        observed = tianxing.normalize_tianxing_observation({
    ...
    def test_route_lease_guard_blocks_non_route_before_downstream_send(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            state_module.state["tianxing_enabled"] = True
            state_module.state["tianxing_observation"] = {
        ...
        allowed_route = tianxing.tianxing_route_pre_send_guard(
            ".野外历练 深入",
            send_as_id=self.identity_id,
            priority="normal",
            intent={"source_module": "野外历练"},
            now=now,
        )
        allowed_small_world = tianxing.tianxing_route_pre_send_guard(
            ".显灵",
            send_as_id=self.identity_id,
            priority="chain",
            intent={"source_module": "小世界"},
            now=now,
        )
        allowed_mulan = tianxing.tianxing_route_pre_send_guard(
            ".支援慕兰 护阵",
            send_as_id=self.identity_id,
            priority="normal",
        ...
        self.assertIn("下一条自动指令必须", blocked["reason"])
    ...
    def test_timeline_plan_probes_panel_only_prediction_before_change_fate_consume(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            state_module.state["tianxing_observation"] = {
                "last_observed_at": now - 60,
    ...
    def test_timeline_plan_reuses_real_prediction_until_it_expires(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            state_module.state["tianxing_observation"] = {
                "last_observed_at": now - 60,
        ...
        self.assertEqual([("release_downstream", "探索")], [(step["action"], step["arg"]) for step in plan["steps"]])

    def test_timeline_plan_calibrates_same_route_prediction_with_missing_until(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            state_module.state["tianxing_observation"] = {
                "last_observed_at": now - 60,
    ...
    def test_timeline_plan_blocks_other_route_prediction_with_missing_until(self):
        ...
        self.assertEqual(0, plan["blocked_until"])
...
class TianxingTimelineSchedulerTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _seed_predict_recent_guard(self, now, command=".推命 闭关"):
        sent_at = float(now - 10)
        action_guard.note_sent(command, self.identity_id, 99002, sent_at=sent_at)
        self.assertTrue(action_guard.close_action("tianxing_predict", send_as_id=self.identity_id, reason="test", now=now - 5))
        return sent_at + action_guard.POST_CLOSE_REPEAT_GUARD_SEC + 2

    async def test_timeline_followup_releases_confirmed_downstream_without_farm(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=34, auto_change=True, dry_run=False)
            state_module.state["tianxing_observation"].update(
                {
                    "current_prediction": "探索",
                    "current_prediction_until": now + 6 * 3600,
                    "current_prediction_set_at": now - 60,
                    "current_change": "探索",
            ...
            state_module.state["tianxing_timeline_state"] = {
                "plan_id": "tianxing-timeline-followup",
            ...
            self.assertTrue(tianxing.has_tianxing_timeline_due_work(now))
            with patch.object(tianxing, "save_state"), patch.object(tianxing, "send_game_command", new=AsyncMock()) as send_mock:
                result = await tianxing.run_tianxing_timeline_followup_scheduler(now)
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
            observed = tianxing.normalize_tianxing_observation(state_module.state["tianxing_observation"])
            route_released = tianxing.is_tianxing_route_released("探索", now=now + 1, require_change_fate=True)
        ...
        self.assertTrue(route_released)
        self.assertEqual("timeline", observed["auto_last_action"])
        self.assertEqual("", observed["auto_last_error"])

    async def test_timeline_followup_does_not_start_craft_when_no_existing_work(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=0, auto_change=False, dry_run=False)
    ...
    def test_craft_farm_daily_limit_resets_across_day_for_legacy_state(self):
        ...
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=35, auto_change=False, dry_run=False)
            state_module.state["tianxing_auto_config"].update(
                {
                    "timeline_enabled": False,
                    "craft_farm_enabled": True,
            ...
            state_module.state["tianxing_timeline_state"] = {
                "craft_farm": {
                    ...
                    "hit_count": 41,
                    "miss_count": 1,
                    "last_result": "炼制攒点今日已达 42 轮。",
                    "audit": [{"ts": yesterday + 3600, "event": "craft_result"}],
                }
            }

            with patch.object(tianxing, "save_state"):
                plan = tianxing.build_tianxing_craft_farm_plan(now=now)
            farm = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])["craft_farm"]

        self.assertNotEqual("daily_limit_reached", plan["stage"])
        self.assertEqual(tianxing.get_day_key(now), farm["daily_day"])
        self.assertEqual(0, farm["daily_count"])
        self.assertEqual(35, farm["estimated_tianji"])

    def test_craft_farm_legacy_daily_limit_wakes_scheduler_even_with_future_auto_next(self):
        now = local_ts(0, 30, year=2026, month=7, day=3)
        yesterday = now - 24 * 3600
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=35, auto_change=False, dry_run=False)
            ...
            observed = tianxing.normalize_tianxing_observation(state_module.state["tianxing_observation"])
            observed["auto_next_time"] = now + 6 * 3600

            should_wake = tianxing._should_wake_tianxing_timeline(observed, config, now)

        self.assertTrue(should_wake)

    async def test_timeline_followup_resets_legacy_craft_daily_limit_without_sending(self):
        now = local_ts(0, 30, year=2026, month=7, day=3)
        yesterday = now - 24 * 3600
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=35, auto_change=False, dry_run=False)
        ...
        send_mock.assert_not_awaited()
        self.assertEqual("daily_counter_reset", result["phase"])
        self.assertEqual(tianxing.get_day_key(now), farm["daily_day"])
        self.assertEqual(0, farm["daily_count"])
    ...
    async def test_timeline_set_star_confirmation_schedules_prompt_replan(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            changed = tianxing.apply_tianxing_passive(
                "你将今日命轨定在【贪狼】。", now=now + 5, family="tianxing_set_star",
                reply_context=record_timeline_test_receipt(self.identity_id),
            )
            observed = tianxing.normalize_tianxing_observation(state_module.state["tianxing_observation"])
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
    ...
    async def test_timeline_waits_for_state_confirmation_before_releasing_route(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            tianxing.apply_tianxing_passive(
                "你为【闭关】推下一段命数，司命盘微微转动。此推命将在 8 小时内生效。", now=now + 6,
                reply_context=record_timeline_test_receipt(self.identity_id),
            )
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
            self.assertEqual("state_confirmed", timeline["phase"])
    ...
    async def test_timeline_ack_timeout_recovers_when_reply_arrived_before_send_return(self):
        ...
        predict_step = {
            ...
            "route": "探索",
            "command": ".推命 探索",
            "status": "confirmed_existing_prediction",
            "send_msg_id": 11482937,
            "sent_at": now - 600,
            "send_started_at": now - 620,
            "confirmed_at": now - 580,
        }
        change_step = {
            "action": "change_fate",
            "arg": "探索",
            ...
            "command": ".改命 探索",
            "status": "ack_timeout",
            "send_msg_id": 11483006,
            # send_game_command may return after the bot reply has already been
            # observed; send_started_at is the safe lower bound for freshness.
            "sent_at": now + 40,
            "send_started_at": now,
            "ack_due_at": now + 55,
            "calibration_due_at": now + 300,
        ...
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=42, auto_change=True, dry_run=False)
            state_module.state["tianxing_observation"].update({
            ...
            state_module.state["tianxing_timeline_state"] = {
                ...
                "route": "探索",
                "active_step_index": 1,
                "active_step": dict(change_step),
                "steps": [dict(predict_step), dict(change_step), dict(release_step)],
                "blocked_until": now + 300,
                "last_error": "天星前置命令回复超时，等待查盘校准；不放行下游。",
            }
            context = record_timeline_test_receipt(self.identity_id)
            state_module.set_game_bot_ids([880530002])
            replies = [{
            ...
            with (
                ...
                patch.object(tianxing, "find_message_log_replies_tail", return_value=replies),
                patch.object(tianxing, "send_game_command") as send_mock,
            ):
                result = await tianxing.run_tianxing_timeline_scheduler(now + 60)
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])

        send_mock.assert_not_called()
        self.assertEqual("downstream_released", result["phase"])
        self.assertEqual(0, timeline["blocked_until"])
        self.assertEqual("released", timeline["active_step"]["status"])
        self.assertTrue(tianxing.is_tianxing_route_released("探索", now=now + 60, require_change_fate=True))

    async def test_timeline_keeps_unknown_send_despite_cached_route_result(self):
        now = 1_780_000_000.0
        active_step = {
            "action": "change_fate",
            "arg": "探索",
        ...
        send_mock.assert_not_called()
        self.assertEqual("calibrating", result["phase"])
        self.assertEqual("panel", timeline["active_step"]["action"])
        self.assertEqual("ack_timeout", timeline["steps"][0]["status"])
        self.assertFalse(timeline["released_routes"])

    async def test_timeline_retains_unknown_operation_after_panel_timeout(self):
        now = 1_780_000_000.0
        route_step = {
            "action": "change_fate",
            "arg": "探索",
        ...
        with state_module.use_identity(self.identity_id), patch.object(tianxing, "send_game_command") as later_send:
            later = await tianxing.run_tianxing_timeline_scheduler(now + 60)
        self.assertEqual("timeline_pending", later["phase"])
        later_send.assert_not_called()
    ...
    async def test_timeline_waits_for_action_guard_short_window_before_send(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, auto_change=False, dry_run=False)
            expected_next_time = self._seed_predict_recent_guard(now)
            with (
                patch.object(tianxing, "save_state"),
        ...
        send_mock.assert_not_awaited()
        self.assertEqual("waiting_send", result["phase"])
        self.assertEqual("waiting_send", timeline["phase"])
        self.assertEqual("pending", timeline["active_step"]["status"])
        self.assertEqual(expected_next_time, timeline["blocked_until"])
        self.assertEqual(expected_next_time, timeline["active_step"]["guard_until"])
        self.assertIn("同命令短窗保护", timeline["last_error"])

    async def test_timeline_send_queue_timeout_defers_for_retry(self):
        now = 1_780_000_000.0
        ...
        self.assertEqual("waiting_send", result["phase"])
        self.assertEqual("pending", timeline["active_step"]["status"])
        self.assertEqual("send_queue_timeout_retry", timeline["audit"][-1]["event"])
    ...
    async def test_timeline_probes_panel_prediction_without_recent_predict(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            with (
                patch.object(tianxing, "save_state"),
                patch.object(tianxing, "send_game_command", return_value=SimpleNamespace(id=9101, chat_id=-100530002, sent_at=now)) as send_mock,
            ):
                result = await tianxing.run_tianxing_timeline_scheduler(now, windows=windows)
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
    ...
    def test_change_fate_consume_window_uses_reliable_minimum_lead(self):
        ...
        self.assertEqual("探索", windows[0]["route"])
        self.assertTrue(windows[0]["require_change_fate"])
        self.assertEqual(now, windows[0]["start_at"])
    ...
    async def test_timeline_change_fate_conflict_does_not_set_long_global_block(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            state_module.state["tianxing_observation"]["current_change_set_at"] = now - 120
            with patch.object(tianxing, "save_state"), patch.object(tianxing, "send_game_command", return_value=SimpleNamespace(id=9101, chat_id=-100530002, sent_at=now)) as send_mock:
                result = await tianxing.run_tianxing_timeline_scheduler(
                    now,
                    windows=[{
                        "route": "探索",
    ...
    async def test_timeline_confirmed_prediction_immediately_releases_downstream(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            self.assertTrue(tianxing.apply_tianxing_passive(
                "你为【探索】推下一段命数，司命盘微微转动。此推命将在 8 小时内生效。", now=now - 1,
                reply_context=record_timeline_test_receipt(self.identity_id),
            ))
            with patch.object(tianxing, "save_state"), patch.object(tianxing, "send_game_command") as send_mock:
                result = await tianxing.run_tianxing_timeline_scheduler(now)
    ...
    async def test_timeline_sending_consumed_state_cannot_release_after_restart(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=35, auto_change=True, dry_run=False)
            state_module.state["tianxing_observation"].update({
                "last_observed_at": now - 1,
                "current_prediction": "探索",
                "current_prediction_until": now + 3600,
    ...
    async def test_timeline_does_not_skip_panel_calibration_from_cached_effects(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=35, auto_change=True, dry_run=False)
            state_module.state["tianxing_observation"].update({
                ...
                "current_prediction": "探索",
                "current_prediction_until": now + 3600,
                "current_prediction_set_at": now - 3,
                "prediction_consumed_route": "探索",
                "prediction_consumed_at": now - 5,
                "last_action": "改命",
                "last_result": "success",
            ...
            state_module.state["tianxing_timeline_state"] = {
                ...
                "steps": [
                    {"action": "predict", "arg": "探索", "route": "探索", "status": "ack_timeout"},
                    {
                        "id": "calibration:test",
                        "action": "panel",
                        "arg": "",
        ...
        send_mock.assert_not_called()
        self.assertEqual("ack_timeout", result["phase"])
        self.assertEqual("ack_timeout", timeline["steps"][1]["status"])
        self.assertEqual("panel", timeline["active_step"]["action"])
        self.assertNotIn("探索", timeline["released_routes"])

    async def test_timeline_sends_its_planned_calibration_without_inventing_release(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_timeline_identity(now, tianji_value=35, auto_change=True, dry_run=False)
            state_module.state["tianxing_observation"].update({
            ...
            with patch.object(tianxing, "save_state"), patch.object(
                tianxing, "send_game_command", new=AsyncMock(return_value=SimpleNamespace(id=9204, sent_at=now)),
            ) as send_mock:
                result = await tianxing.run_tianxing_timeline_scheduler(now)
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
    ...
    async def test_timeline_panel_calibration_without_target_replans_and_resends(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            context = record_timeline_test_receipt(self.identity_id)
            original = state_module.state["tianxing_timeline_state"]["steps"][0]
            original.update(send_account_id=state_module.get_identity_account(self.identity_id), send_chat_id=context["chat_id"])
            action_guard.note_sent(".天机盘", self.identity_id, 9102, sent_at=now - 3, chat_id=context["chat_id"])
            state_module.state["pending_tasks"][(context["chat_id"], 9102)] = {
                "cmd": ".天机盘", "chat_id": context["chat_id"], "send_started_at": now - 4, "sent_at": now - 3,
            }
            self.assertTrue(tianxing.apply_tianxing_passive(
                "【天机盘】\n当前推命: 无\n当前改命: 无\n天机值: 12\n逆命劫: 0", now=now - 1,
    ...
    async def test_ack_timeout_ignores_external_panel_cache_without_source(self):
        now = 1_780_000_000.0
        sent_at = now - 120
        with state_module.use_identity(self.identity_id):
    ...
    def test_timeline_rejects_unique_unthreaded_prediction_reply_from_log(self):
        now = 1_780_000_000.0
        sent_at = now - 120
        reply_text = (
            "你拨动司命盘，为 【斗法】 推下一段命数。\n"
        ...
        with state_module.use_identity(self.identity_id):
            ...
            record_timeline_test_receipt(self.identity_id)
            state_module.set_game_bot_ids([880530002])
            with patch.object(
                tianxing,
                ...
                changed = tianxing._recover_tianxing_timeline_reply_from_message_log(now)
            observed = tianxing.normalize_tianxing_observation(state_module.state["tianxing_observation"])
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
    ...
    async def test_timeline_ignores_stale_reply_before_sent_at(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            tianxing.apply_tianxing_passive(
                "你为【闭关】推下一段命数，司命盘微微转动。此推命将在 8 小时内生效。", now=now - 5,
                reply_context=record_timeline_test_receipt(self.identity_id),
            )
            timeline = tianxing.normalize_tianxing_timeline_state(state_module.state["tianxing_timeline_state"])
    ...
    async def test_timeline_late_reply_after_timeout_releases_without_panel(self):
        now = 1_780_000_000.0
        first_msg = SimpleNamespace(id=9101, sent_at=now)
        ...
        with state_module.use_identity(self.identity_id):
            ...
            tianxing.apply_tianxing_passive(
                "你为【闭关】推下一段命数，司命盘微微转动。此推命将在 8 小时内生效。", now=now + 20,
                reply_context=record_timeline_test_receipt(self.identity_id),
            )
            ...
            with patch.object(tianxing, "save_state"):
                released = await tianxing.run_tianxing_timeline_scheduler(now + 21, windows=self._farm_windows(now))

        self.assertEqual("downstream_released", released["phase"])
        self.assertTrue(tianxing.is_tianxing_route_released("闭关", now=now + 21))
...
class TianxingRetreatFarmTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _seed_craft_recent_guard(self, now, command=".炼制 玄铁剑"):
        sent_at = float(now - 10)
        action_guard.note_sent(command, self.identity_id, 99001, sent_at=sent_at)
        self.assertTrue(action_guard.close_action("tianxing_craft_farm", send_as_id=self.identity_id, reason="test", now=now - 5))
        return sent_at + action_guard.POST_CLOSE_REPEAT_GUARD_SEC + 2
    ...
    def test_craft_farm_runs_at_normal_interval_outside_legacy_window(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            state_module.state["tianxing_observation"]["current_prediction_set_at"] = now - 60
            state_module.state["tianxing_timeline_state"] = {
                "released_routes": {
    ...
    async def test_craft_farm_waits_for_action_guard_short_window_before_send(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_identity(now, tianji_value=3)
            state_module.state["tianxing_observation"]["current_prediction"] = "炼制"
            ...
            expected_next_time = self._seed_craft_recent_guard(now)
            with (
                patch.object(tianxing, "send_game_command", new=AsyncMock()) as send_mock,
                patch.object(tianxing, "save_state"),
            ):
        ...
        self.assertEqual("action_guard_waiting", result["stage"])
        self.assertEqual("", result["command"])
        send_mock.assert_not_awaited()
        self.assertEqual("ready", craft["phase"])
        self.assertEqual("action_guard_waiting", craft["last_result"])
        self.assertEqual(expected_next_time, craft["next_time"])
        self.assertIn("同命令短窗保护", craft["last_error"])
    ...
    async def test_consume_craft_prediction_waits_for_action_guard_short_window_before_send(self):
        now = 1_780_000_000.0
        with state_module.use_identity(self.identity_id):
            self._prepare_identity(now, tianji_value=32)
            state_module.state["tianxing_observation"]["current_prediction"] = "炼制"
    ...
    async def test_craft_farm_consumes_released_route_even_when_old_timeline_wait_is_future(self):
        now = 1_780_000_000.0
        sent_msg = SimpleNamespace(id=9403, sent_at=now)
        with state_module.use_identity(self.identity_id):
            self._prepare_identity(now, tianji_value=12)
            state_module.state["tianxing_observation"].update({
            ...
            state_module.state["tianxing_timeline_state"] = {
                ...
                "released_routes": {
                    "炼制": {"released_at": now - 4, "plan_id": "test", "reason": "炼制已放行", "basis": "prediction"}
                },
                "craft_farm": {
                    "phase": "timeline_waiting",
                    "next_time": now + 90,
    ...
    async def test_craft_farm_send_queue_timeout_uses_staggered_retry(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            with (
                patch.object(tianxing, "send_game_command", new=AsyncMock(return_value=None)) as send_mock,
                patch.object(tianxing, "get_last_game_send_block", return_value={"code": "send_queue_timeout"}),
                patch.object(tianxing.random, "uniform", return_value=33),
                patch.object(tianxing, "save_state"),
            ):
                result = await tianxing.run_tianxing_craft_farm_scheduler(
                    now,
    ...
    async def test_craft_farm_preserves_explore_prediction_conflict_without_bare_retry(self):
        ...
        async def mark_explore_conflict(*_args, **_kwargs):
            state_module.state["tianxing_observation"]["current_prediction"] = "探索"
            state_module.state["tianxing_observation"]["current_prediction_until"] = now + 3600
            state_module.state["tianxing_observation"]["current_prediction_set_at"] = now
            state_module.state["tianxing_timeline_state"] = {
                "phase": "prediction_conflict",
                "route": "炼制",
                "blocked_until": now + 3600,
    ...
    def test_craft_farm_override_conflict_wakes_fast_scan(self):
        ...
        with state_module.use_identity(self.identity_id):
            ...
            self.assertTrue(tianxing.has_tianxing_craft_farm_override_due(now))
