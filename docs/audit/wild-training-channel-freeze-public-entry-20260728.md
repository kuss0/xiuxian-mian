...
2026-07-28 04:39:44 UTC+8 重启部署后，公共入口继续按单 worker 串行补跑，至
04:53:42 完成收口：

- 16 个非天星频道身份全部达到当日 `daily_count=2`、`daily_remaining=0`。
- WA、Lsfnqy 两个可用个人身份也保持当日 `2/2`，合计 18 个当日记录全部耗尽次数。
- 4 个天星频道身份均进入“频道身份冻结，等待天星探索前置恢复”，没有发送
  `.天机盘`、`.推命`、`.改命` 或旧 `.野外历练`。
- 重启后 journal 未出现 `MiniApp 野外历练未完成`、HTTP 429/5xx、身份错绑、重复结算
  或 Python 异常。
- `channel-frozen public wild-training lag` 已自动消失；当前只保留 listener sidecar
  缺独立授权账号的已知冗余告警。
